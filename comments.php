...
<p class="warning-text" style="margin-bottom:10px">以<a class="layui-badge" href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>身份登录 | <a class="link-logout" href="<?php echo wp_logout_url(get_permalink()); ?>">注销</a></p>
...
<div class="comment-form-url form-group has-feedback"><div class="input-group"><input class="form-control" required="required" placeholder="<?php echo  __('人机验证：','moedog').$comment_num1?> ＋ <?php echo $comment_num2;?> = ?" id="code" name="code" type="text" value="" autocomplete="off" /><input type="hidden" name="co_num1" value="<?php echo ($comment_num1 + 1);?>" /><input type="hidden" name="co_num2" value="<?php echo  ($comment_num2 + 2);?>" /></div></div>
</label>
</div>
<?php endif; ?>
